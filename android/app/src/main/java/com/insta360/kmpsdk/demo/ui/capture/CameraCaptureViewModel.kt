...
import com.insta360.kmpsdk.demo.touchscene.CaptureTerminalGate
import com.insta360.kmpsdk.demo.touchscene.RecordingStartGate
...
sealed interface CaptureUiEffect {
    data class RecordingStarted(val functionMode: FunctionMode) : CaptureUiEffect
    data class CaptureFinished(val functionMode: FunctionMode) : CaptureUiEffect
    data class CaptureFailed(val functionMode: FunctionMode, val nativeErrorCode: Int?, val errorType: String) : CaptureUiEffect
...
class CameraCaptureViewModel(
    ...
    private val captureTerminalGate = CaptureTerminalGate()
    private val recordingStartGate = RecordingStartGate()
    ...
    private suspend fun performLensSwitchCore(
        ...
        try {
            ...
            if (_ui.value.busy) setBusy(false)
        }
    }
...
//            cap.syncAllParams()
    ...
    fun onPrimaryCaptureButtonClicked() {
        ...
        if (!provisionalStop) {
            captureTerminalGate.begin()
            if (mode.functionType == FunctionType.VIDEO) recordingStartGate.begin()
            else recordingStartGate.invalidate()
        } else {
        ...
        viewModelScope.launch {
            ...
            if (stopNow) recordingStartGate.invalidate()
            Timber.d(
            ...
            val actionOk =
                runCatching {
                    ...
                    .isSuccess
            if (!actionOk) recordingStartGate.invalidate()
    ...
    private fun markRecordingStarted(functionMode: FunctionMode) {
        if (functionMode.functionType != FunctionType.VIDEO) return
        val state = _ui.value
        if (recordingStartGate.claim(
                active = state.captureFlowActive && state.selectedMode == functionMode,
                stopping = state.captureCommand == CaptureCommandPhase.STOPPING,
            )
        ) {
            _effect.tryEmit(CaptureUiEffect.RecordingStarted(functionMode))
        }
    }
        ...
        val listener =
            object : CaptureStatusListener {
                ...
                override fun onCaptureWorking(functionMode: FunctionMode) {
                    markRecordingStarted(functionMode)
                ...
                override fun onCaptureFinish(
                    ...
                    val active = _ui.value.let { it.captureFlowActive || it.captureCommand != null }
                    if (!captureTerminalGate.claim(active)) {
                        Timber.w("Ignore duplicate or stale capture finish mode=%s", functionMode)
                        return
                    }
                    recordingStartGate.invalidate()
                ...
                override fun onCaptureError(
                    ...
                    val active = _ui.value.let { it.captureFlowActive || it.captureCommand != null }
                    if (!captureTerminalGate.claim(active)) {
                        Timber.w(throwable, "Ignore duplicate or stale capture error mode=%s", functionMode)
                        return
                    }
                    recordingStartGate.invalidate()
                    Timber.w(throwable, "onCaptureError mode=%s", functionMode)
                    _effect.tryEmit(
                        CaptureUiEffect.CaptureFailed(
                            functionMode,
                            (throwable as? NativeException)?.nativeErrorCode,
                ...
                override fun onCaptureSubStatusChanged(
                    ...
                    when (subStatus) {
                        ...
                        CameraCaptureStatus.SubStatus.PHOTO_CANCEL,
                            -> {
                            captureTerminalGate.invalidate()
                            recordingStartGate.invalidate()
                            _ui.update {
                                val mode = it.selectedMode
                                val (label, enabled) =
